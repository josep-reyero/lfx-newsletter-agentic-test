...
// decodeGroupStatusReply decodes a raw lfx.email-service.get_email_status
// group-status reply into normalized port records. It is the contract-bearing
// parse path: an empty reply means "no records yet" (nil, nil), a JSON error
// envelope surfaces as a ServiceUnavailable error, and a JSON array of
// recipient records is mapped via toPortRecord (handling both the flat
// opened_at and the opened_at_list shapes). Split out from GetStatusByGroupID
// so it can be unit-tested with representative email-service replies without a
// live NATS connection.
func decodeGroupStatusReply(reply []byte) ([]port.EmailRecipientRecord, error) {
	if len(reply) == 0 {
...
func (d *EmailDispatcher) GetStatusByGroupID(ctx context.Context, groupID string) ([]port.EmailRecipientRecord, error) {
	...
	return decodeGroupStatusReply(reply)
}
