...
func TestAnalyticsGet_DailyOpensFromGroupStatus(t *testing.T) {
	...
	if got.TotalOpens != 3 {
		t.Errorf("TotalOpens: got %d, want 3 (per-event series; one event per recipient)", got.TotalOpens)
	}
...
func TestAnalyticsGet_CountsPerEventOpens(t *testing.T) {
	...
	// TotalOpens is sourced from the per-event series (4 = a1,a2,a3,b1), not the
	// scalar engagement.Opened, and must equal the sum of DailyOpens.Opens so
	// the public contract holds.
	if got.TotalOpens != 4 {
		t.Errorf("TotalOpens: got %d, want 4 (sum of per-event opens)", got.TotalOpens)
	}
	if len(got.DailyOpens) != 2 {
		t.Fatalf("DailyOpens length: got %d, want 2", len(got.DailyOpens))
	}
	sumDaily := 0
	for _, d := range got.DailyOpens {
		sumDaily += d.Opens
	}
	if got.TotalOpens != sumDaily {
		t.Errorf("TotalOpens (%d) must equal sum of DailyOpens.Opens (%d)", got.TotalOpens, sumDaily)
	}
	if got.UniqueOpens > got.TotalOpens {
		t.Errorf("UniqueOpens (%d) must be <= TotalOpens (%d)", got.UniqueOpens, got.TotalOpens)
	}
